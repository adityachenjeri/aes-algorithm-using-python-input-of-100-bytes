#AES S-Box
s_box = [
    
]

# AES Round Constants
round_constants = [
    [0x01, 0x00, 0x00, 0x00],
    [0x02, 0x00, 0x00, 0x00],
    [0x04, 0x00, 0x00, 0x00],
    [0x08, 0x00, 0x00, 0x00],
    [0x10, 0x00, 0x00, 0x00],
    [0x20, 0x00, 0x00, 0x00],
    [0x40, 0x00, 0x00, 0x00],
    [0x80, 0x00, 0x00, 0x00],
    [0x1B, 0x00, 0x00, 0x00],
    [0x36, 0x00, 0x00, 0x00]
]

def sub_bytes(state):
    for i in range(4):
        for j in range(4):
            state[i][j] = s_box[state[i][j]]

def shift_rows(state):
    for i in range(1, 4):
        state[i] = state[i][i:] + state[i][:i]

def mix_columns(state):
    for i in range(4):
        a = state[i][0]
        b = state[i][1]
        c = state[i][2]
        d = state[i][3]

        state[i][0] = (
            (0x02 * a) ^ (0x03 * b) ^ c ^ d
        ) & 0xFF
        state[i][1] = (
            a ^ (0x02 * b) ^ (0x03 * c) ^ d
        ) & 0xFF
        state[i][2] = (
            a ^ b ^ (0x02 * c) ^ (0x03 * d)
        ) & 0xFF
        state[i][3] = (
            (0x03 * a) ^ b ^ c ^ (0x02 * d)
        ) & 0xFF

def add_round_key(state, key):
    for i in range(4):
        for j in range(4):
            state[i][j] ^= key[i][j]

def expand_key(key):
    key_schedule = [[key[i + j] for j in range(4)] for i in range(0, 16, 4)]

    for i in range(4, 44):
        temp = list(key_schedule[i - 1])

        if i % 4 == 0:
            temp = temp[1:] + [temp[0]]
            for j in range(4):
                temp[j] = s_box[temp[j]]
            temp[0] ^= round_constants[i // 4 - 1][0]

        key_schedule.append([
            (key_schedule[i - 4][j] ^ temp[j]) & 0xFF
            for j in range(4)
        ])

    return key_schedule

def pad_data(data):
    padding_length = 16 - (len(data) % 16)
    padded_data = data + bytes([padding_length] * padding_length)
    return padded_data

def encrypt_block(input_data, key):
    state = [[input_data[i + j] for j in range(4)] for i in range(0, 16, 4)]

    key_schedule = expand_key(key)

    add_round_key(state, key_schedule)

    for round_num in range(1, 10):
        sub_bytes(state)
        shift_rows(state)
        mix_columns(state)
        add_round_key(state, key_schedule)

        key_schedule = [
            [
                (key_schedule[i][j] ^ key_schedule[i - 4][j] ^ round_constants[round_num][j]) & 0xFF
                for j in range(4)
            ]
            for i in range(4)
        ]

    sub_bytes(state)
    shift_rows(state)
    add_round_key(state, key_schedule)

    return [state[i][j] for i in range(4) for j in range(4)]

def encrypt(data, key):
    padded_data = pad_data(data)
    encrypted_data = bytearray()

    for i in range(0, len(padded_data), 16):
        block = padded_data[i:i+16]
        encrypted_block = encrypt_block(block, key)
        encrypted_data.extend(encrypted_block)

    return encrypted_data

# Usage Example
plaintext = "The quick brown fox jumps over the lazy dog. 1234567890!@#$%^&*()-=_+[]{}|;':,.<>?/`~"
key = b"This is a sample key"

encrypted_data = encrypt(plaintext.encode(), key)
output = repr(encrypted_data)
print(output)
